...
'''
Script build_events.py

A python script to builds clinical events from feeders data and Patient Activity data.
    ...
    REQUIRED
    hospital_code
    The hospital code for the hospital whose clinical events
    is being built.

    model_code
    The model code for the clinical costing model that is being used
    to build the clinical events.

    run_code
    The run code for the source data being used
    to build the clinical events for this hospital.
...
if __name__ == '__main__':
    ...
    Then check that the hospital_code, model_code and run_code are valid.
    Then build the clinical events.
    '''
